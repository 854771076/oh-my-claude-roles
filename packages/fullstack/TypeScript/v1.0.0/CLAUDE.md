...
### Hooks (钩子脚本)
- `.claude/hooks/eslint-check.json` - 自动执行ESLint代码检查
- `.claude/hooks/prettier-format.json` - 自动执行Prettier代码格式化
- `.claude/hooks/typescript-compile-check.json` - 自动执行TypeScript编译检查
- `.claude/hooks/unit-test-check.json` - 自动执行单元测试检查
- `.claude/hooks/security-audit.json` - 自动执行依赖安全漏洞审计
- `.claude/hooks/conventional-commits-check.json` - 自动检查提交信息是否符合Conventional Commits规范
- `.claude/hooks/env-check.json` - 自动检查环境变量配置完整性
- `.claude/hooks/naming-convention-check.json` - 自动检查文件和代码命名是否符合规范
- `.claude/hooks/any-type-check.json` - 自动检查代码中是否存在禁止使用的any类型
- `.claude/hooks/ts-ignore-check.json` - 自动检查代码中是否存在禁止使用的@ts-ignore注释
- `.claude/hooks/server-component-check.json` - 自动检查服务端组件是否符合规范

### Commands (斜杠命令)
- `.claude/commands/init-nextjs-project.md` - 初始化符合企业级规范的Next.js项目
- `.claude/commands/create-component.md` - 创建符合规范的Next.js组件
- `.claude/commands/create-server-action.md` -